...
export default function WorkspaceEditorPage() {
  ...
  const DEFAULT_CHAPTER_TEMPLATES = {
    bab1: {
      struktur: "1.1 Latar Belakang, 1.2 Rumusan Masalah, 1.3 Tujuan Penelitian, 1.4 Manfaat Penelitian",
      panduan: "Latar belakang harus memuat fenomena umum, data/fakta lapangan di lokasi penelitian, research gap, serta urgensi masalah."
    },
    bab2: {
      struktur: "2.1 Landasan Teori, 2.2 Tinjauan Pustaka / Penelitian Terdahulu, 2.3 Kerangka Pemikiran, 2.4 Hipotesis Penelitian",
      panduan: "Uraikan variabel-variabel penelitian berdasarkan kutipan literatur. Bandingkan minimal 3 jurnal terdahulu, lalu susun alur hubungan antar variabel dalam kerangka pemikiran."
    },
    bab3: {
      struktur: "3.1 Pendekatan & Jenis Penelitian, 3.2 Populasi & Sampel, 3.3 Teknik Pengumpulan Data, 3.4 Instrumen Penelitian, 3.5 Teknik Analisis Data",
      panduan: "Jelaskan metode penelitian (kuantitatif/kualitatif), jumlah target populasi dan sampel, prosedur pengumpulan data, serta rumus statistik atau model analisis yang digunakan."
    },
    bab4: {
      struktur: "4.1 Gambaran Umum Subjek, 4.2 Deskripsi Data & Hasil Analisis, 4.3 Pembahasan Temuan",
      panduan: "Paparkan hasil pengumpulan data dan analisis statistik (atau temuan wawancara). Hubungkan temuan ini dengan teori di Bab II untuk melakukan pembahasan akademik."
    },
    bab5: {
      struktur: "5.1 Kesimpulan, 5.2 Implikasi Penelitian, 5.3 Saran",
      panduan: "Tarik kesimpulan singkat yang langsung menjawab rumusan masalah. Jelaskan implikasi akademis/praktis, dan berikan saran spesifik bagi pembaca atau peneliti selanjutnya."
    }
  };
        ...
        if (isMounted) {
          ...
          if (!hydratedRef.current) {
            ...
            CHAPTERS.forEach((chapter) => {
              initialBuffer[chapter.key] = deserializeLocalImages(data[chapter.key] || "");
            });
  ...
  useEffect(() => {
    ...
    async function fetchSubs() {
      try {
        ...
        CHAPTERS.forEach(ch => {
          const chNote = allNotes.find(n => n.workspace_id === id && n.id === `chapter_context_${ch.key}_${id}`);
        ...
        if (isMounted && !hasCheckedOnboarding) {
          ...
          const hasNoCh = !loadedChapterContexts[currentChKey]?.struktur && !loadedChapterContexts[currentChKey]?.panduan;
        ...
        <div className="workspace-scroll" style={{ display: "flex", flexDirection: "column", gap: "1rem", overflowY: "auto", flex: 1, minHeight: 0, paddingRight: "0.25rem" }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setEditedChapterContext({
                      struktur: chapterContexts[currentChapter.key]?.struktur || DEFAULT_CHAPTER_TEMPLATES[currentChapter.key]?.struktur || "",
                      panduan: chapterContexts[currentChapter.key]?.panduan || DEFAULT_CHAPTER_TEMPLATES[currentChapter.key]?.panduan || ""
                    });
          ...
          <div className="glass-panel" style={{ padding: isSm ? "0.65rem" : "1rem", backgroundColor: "var(--surface)", display: "flex", flexDirection: "column", gap: isSm ? "0.5rem" : "0.75rem" }}>
            ...
            {isEditingContext ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label" style={{ fontSize: isSm ? "0.68rem" : "0.72rem", marginBottom: "0.15rem" }}>Struktur Bab (Placeholder/Rencana Bagian)</label>
                  <textarea
                    className="form-input"
                    style={{ fontSize: isSm ? "0.74rem" : "0.78rem", padding: isSm ? "0.3rem" : "0.4rem" }}
                    rows={2}
                    ...
                    onChange={e => setEditedChapterContext(prev => ({ ...prev, struktur: e.target.value }))}
                  />
                </div>
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label" style={{ fontSize: isSm ? "0.68rem" : "0.72rem", marginBottom: "0.15rem" }}>Panduan Isi Bab</label>
                  <textarea
                    className="form-input"
                    style={{ fontSize: isSm ? "0.74rem" : "0.78rem", padding: isSm ? "0.3rem" : "0.4rem" }}
                    rows={3}
                    ...
                    onChange={e => setEditedChapterContext(prev => ({ ...prev, panduan: e.target.value }))}
                  />
                </div>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", fontSize: isSm ? "0.72rem" : "0.78rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
                <div><strong>Struktur Bab:</strong> {chapterContexts[currentChapter.key]?.struktur || DEFAULT_CHAPTER_TEMPLATES[currentChapter.key]?.struktur}</div>
                <div><strong>Panduan Isi:</strong> {chapterContexts[currentChapter.key]?.panduan || DEFAULT_CHAPTER_TEMPLATES[currentChapter.key]?.panduan}</div>
              </div>
            )}
          </div>
              ...
              {showSaveConfirm ? (
                <div className="glass-panel" style={{ padding: "0.75rem", backgroundColor: "rgba(79, 70, 229, 0.05)", border: "1px solid var(--primary)", borderRadius: "8px", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                    <button className="btn btn-ghost" style={{ padding: "0.25rem 0.5rem", fontSize: "0.7rem", height: "auto" }} onClick={() => setShowSaveConfirm(false)}>Batal</button>
                    <button className="btn btn-primary" style={{ padding: "0.25rem 0.5rem", fontSize: "0.7rem", height: "auto" }} onClick={() => handleSaveContext(editedRootContext, editedChapterContext)}>Ya, Simpan</button>
                  </div>
                </div>
              ) : (
                <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                  <button className="btn btn-outline" style={{ padding: "0.35rem 0.75rem", fontSize: "0.75rem" }} onClick={() => setIsEditingContext(false)}>Batal</button>
                  <button className="btn btn-primary" style={{ padding: "0.35rem 0.75rem", fontSize: "0.75rem" }} onClick={() => setShowSaveConfirm(true)}>Simpan Konteks</button>
                </div>
              )}
            </div>
          )}
    ...
    <div style={{ margin: isMobile ? "-0.75rem" : "-1.5rem", minHeight: "calc(100vh - 72px)", backgroundColor: "var(--background)" }}>
      ...
      >
        <div style={{ display: "flex", flexDirection: isXs ? "column" : "row", flexWrap: "wrap", justifyContent: "space-between", gap: isXs ? "0.6rem" : "0.9rem", alignItems: isXs ? "stretch" : "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "0.75rem", justifyContent: "flex-end", flexShrink: 0 }}>
      ...
      <ChapterAiAssistant
        ...
        chapterContext={chapterContexts[currentChapter?.key] || DEFAULT_CHAPTER_TEMPLATES[currentChapter?.key] || {}}
        isMobile={isSm}
        ...
        >
          ...
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: "0.86rem", color: "var(--text-muted)", lineHeight: 1.6 }}>
              Sebelum mulai menulis dengan AI, lengkapi dulu <strong>Konteks Utama</strong> agar draf akademik yang dihasilkan terarah dan akurat.
