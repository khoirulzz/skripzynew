...
export async function approveTopup(requestOrId, userId, amount) {
  try {
    ...
    // Create Notification
    const notifMsg = safeRequestType === "plan" 
      ? `Pembayaran manual untuk paket ${requestData.planName || "Langganan"} telah disetujui. Akun Anda telah di-upgrade.`
      : `Pembayaran manual sebesar Rp ${(requestData.amount || amount || 0).toLocaleString("id-ID")} telah disetujui. Saldo kredit Anda telah ditambahkan.`;
    ...
    }).catch(e => console.error("Gagal mengirim notifikasi topup approved", e));
    ...
    if (requestData && requestData.userId) {
      await d1Request("notifications", {
        ...
        body: {
          ...
          title: "Top-up Gagal",
          message: `Pembayaran manual Anda gagal atau ditolak. Alasan: ${reason || "Silakan hubungi admin."}`,
          type: "transaction",
          isRead: 0,
          actionUrl: "/dashboard/langganan"
        }
      }).catch(e => console.error("Gagal mengirim notifikasi topup rejected", e));
    }
