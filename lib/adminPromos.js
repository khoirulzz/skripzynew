...
export async function createPromo(data) {
  try {
    ...
    const body = {
      ...
      showOnLanding: data.showOnLanding !== false ? 1 : 0,
      applicableTo: data.applicableTo || 'all',
      minTransaction: data.minTransaction ? parseInt(data.minTransaction, 10) : 0,
...
export async function updatePromo(id, data) {
  try {
    ...
    if (typeof body.showOnLanding === "boolean") body.showOnLanding = body.showOnLanding ? 1 : 0;
    if (body.minTransaction !== undefined) body.minTransaction = parseInt(body.minTransaction, 10) || 0;
 ...
 */
export async function validatePromoCode(code, orderType = "all", orderAmount = 0) {
  try {
    const { data } = await d1Request("promos", { method: "GET" });
    const promos = data || [];
    ...
    const promo = {
        ...promoMatch,
        discountPercent: promoMatch.type === "percent" ? promoMatch.discountValue : undefined,
        discountAmount: promoMatch.type === "fixed" ? promoMatch.discountValue : undefined,
    ...
    if (promo.applicableTo && promo.applicableTo !== "all" && promo.applicableTo !== orderType) {
        throw new Error(`Promo ini hanya berlaku untuk ${promo.applicableTo === "plan" ? "Plan Langganan" : "Top Up Kredit"}.`);
    }

    if (promo.minTransaction && promo.minTransaction > 0 && orderAmount < promo.minTransaction) {
        throw new Error(`Minimal transaksi untuk menggunakan promo ini adalah Rp ${promo.minTransaction.toLocaleString("id-ID")}.`);
    }
