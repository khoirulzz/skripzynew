...
export const PAYMENT_METHODS = [
  {
    ...
    description: "Checkout instan via iPaymu gateway (QRIS, VA, E-Wallet).",
    enabled: false,
...
export const MANUAL_PAYMENT_CHANNELS = [
  {
    ...
    group: "qris",
    label: "QRIS",
    helper: "A.n. SKRIPZY DIGITAL & EDUCATION — Scan dari mobile banking atau e-wallet",
    value: "Scan QR Code",
    accent: "#8B5CF6",
    icon: "sparkles",
    logo: "/payment/qris-logo.svg",
    qrisImage: "/QRIS.jpeg",
  },
  {
    id: "bank-seabank",
    group: "bank",
    label: "SeaBank",
    helper: "A.n. Muhamad Khoirul Ulum",
    value: "901312151496",
    accent: "#00B4D8",
    icon: "creditCard",
...
export function calculatePromoBreakdown(basePrice, promo) {
  ...
  const isFixed = promo?.type === "fixed" || promo?.type === "nominal";
  
  // Fallback to discountValue if mapped fields are missing
  const rawAmount = promo?.discountAmount ?? (isFixed ? promo?.discountValue : 0);
  const rawPercent = promo?.discountPercent ?? (!isFixed ? promo?.discountValue : 0);

  const discountAmount = isFixed
    ? toNumber(rawAmount)
    : Math.round(safeBasePrice * (toNumber(rawPercent) / 100));
...
export async function createIpaymuPayment({
  ...
  const idToken = await user.getIdToken();
  const workerUrl = process.env.NEXT_PUBLIC_WORKER_URL || "https://apikey.skripzy-app.workers.dev";
  const endpoint = `${workerUrl}/api/ipaymu/create-payment`;
  ...
  const payload = {
    ...
    customerName: userData?.namaLengkap || userData?.displayName || user.displayName || user.email || "User Skripzy",
    customerEmail: userData?.email || user.email || "",
    customerPhone: userData?.phone || userData?.phoneNumber || userData?.whatsapp || "",
    requestType,
    productName: requestType === "plan" ? getPlanDisplayName(selectedItem.planId) : selectedItem.name,
    promoId: promo?.id || null,
    promoCode: promo?.code || null,
...
export async function createBillingRequest({
  ...
  const id = "topup_" + Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
  payload.id = id;
...
export function subscribeToUserBillingRequests(userId, callback) {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const userRequests = (data || []).filter(item => item.userId === userId);
      callback(sortByTimestampDesc(userRequests));
    } catch (e) {}
  };
...
export async function updateBillingRequest(id, fields) {
  if (!id) throw new Error("ID permintaan tidak valid.");
  await d1Request("topups", {
    method: "PATCH",
    id,
    body: fields
...
export function generateDynamicQRIS(baseQRIS, amount) {
  let baseWithoutCRC = baseQRIS.slice(0, -8);
  ...
  const tag53 = "5303360";
  const index = baseWithoutCRC.indexOf(tag53);
  if (index !== -1) {
    const insertPos = index + tag53.length;
    baseWithoutCRC = baseWithoutCRC.slice(0, insertPos) + tag54 + baseWithoutCRC.slice(insertPos);
  } else {
    baseWithoutCRC = baseWithoutCRC + tag54;
  }
  
  const checksum = calculateCRC16(baseWithoutCRC + "6304");
  return baseWithoutCRC + "6304" + checksum;
}

export async function checkDuplicateReference(referenceNumber, currentRequestId = null) {
  ...
  try {
    ...
    return (data || []).some(
      (item) => 
        ...
        (!currentRequestId || item.id !== currentRequestId)
    );
  } catch (error) {
...
export function analyzeReceiptText({ ocrText, confidence, targetPrice, paymentChannelId }) {
  const normalizedText = ocrText.toLowerCase().replace(/[^a-z0-9]/g, "");
  const normalizedTargetPrice = String(targetPrice).replace(/[^0-9]/g, "");
  
  let score = 0;
  let reasons = [];
  let extractedRef = "";
  
  // 1. Nominal Check (50 pts)
  if (normalizedText.includes(normalizedTargetPrice)) {
    score += 50;
    reasons.push("Nominal sesuai (+50)");
  } else {
    reasons.push("Nominal tidak cocok");
  }

  // 2. Merchant & Channel Check (30 pts)
  let channelMatch = false;
  if (normalizedText.includes("muhamadkhoirulum") || normalizedText.includes("skripzy")) {
    score += 15;
    reasons.push("Nama merchant/penerima sesuai (+15)");
  }

  if (paymentChannelId === "qris-main") {
    if (normalizedText.includes("qris") || normalizedText.includes("gopay") || normalizedText.includes("shopeepay") || normalizedText.includes("dana") || normalizedText.includes("ovo") || normalizedText.includes("linkaja") || normalizedText.includes("pekalongan")) {
      channelMatch = true;
    }
  } else if (paymentChannelId === "bank-seabank") {
    if (normalizedText.includes("seabank") || normalizedText.includes("sea") || normalizedText.includes("901312151496")) {
      channelMatch = true;
    }
    if (normalizedText.includes("901312151496")) {
      score += 15;
      reasons.push("Nomor rekening SeaBank sesuai (+15)");
    }
  } else if (paymentChannelId === "bank-bri") {
    if (normalizedText.includes("bri") || normalizedText.includes("rakyat") || normalizedText.includes("597201037038539")) {
      channelMatch = true;
    }
    if (normalizedText.includes("597201037038539")) {
      score += 15;
      reasons.push("Nomor rekening BRI sesuai (+15)");
    }
  } else if (["ewallet-gopay", "ewallet-shopeepay", "ewallet-dana"].includes(paymentChannelId)) {
    if (normalizedText.includes("083120039389") || normalizedText.includes("83120039389")) {
      channelMatch = true;
      score += 15;
      reasons.push("Nomor e-wallet sesuai (+15)");
    }
  }

  if (channelMatch) {
    score += 15;
    reasons.push("Metode/saluran transfer sesuai (+15)");
  } else {
    reasons.push("Metode/saluran transfer tidak cocok");
  }

  // 3. Status Success Check (15 pts)
  const successKeywords = ["berhasil", "sukses", "success", "lunas", "selesai", "diterima", "transferberhasil", "pembayaranberhasil", "paymentsuccess"];
  const hasSuccessKeyword = successKeywords.some(kw => normalizedText.includes(kw));
  if (hasSuccessKeyword) {
    score += 15;
    reasons.push("Status transaksi sukses (+15)");
  } else {
    reasons.push("Kata kunci sukses tidak ditemukan");
  }

  // 4. Date check (5 pts)
  if (normalizedText.includes("2026") || normalizedText.includes("may") || normalizedText.includes("mei")) {
    score += 5;
    reasons.push("Waktu transaksi valid (+5)");
  }

  // 5. OCR Quality check
  if (confidence < 60) {
    score = Math.max(0, score - 20);
    reasons.push("Kualitas pembacaan OCR rendah (-20)");
  }

  // Extract reference number
  const refRegexes = [
    /ref(?:erensi)?\s*[:\-\s]\s*([a-z0-9\-]+)/i,
    /id\s*transaksi\s*[:\-\s]\s*([a-z0-9\-]+)/i,
    /no\s*(?:\.\s*)?resi\s*[:\-\s]\s*([a-z0-9\-]+)/i,
    /reference\s*(?:no|number)?\s*[:\-\s]\s*([a-z0-9\-]+)/i,
    /no\s*(?:\.\s*)?ref\s*[:\-\s]\s*([a-z0-9\-]+)/i,
  ];
  ...
  if (!extractedRef) {
    const numMatch = ocrText.match(/\b\d{8,18}\b/);
    if (numMatch) {
