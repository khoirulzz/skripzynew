...
async function _call({ prompt, history, systemInstruction, model, group, temperature, thinkingConfig, attempt = 0 }) {
  ...
  if (!res.ok) {
    ...
    if (nextIndex < fallbackChain.length) {
      ...
      return _call({ prompt, history, systemInstruction, model: fallbackChain[nextIndex], group, temperature, thinkingConfig, attempt: nextIndex });
    }
...
export async function callGemini({
  ...
  for (const groupName of groupsToTry) {
    try {
      return await _call({ prompt, history, systemInstruction, model, group: groupName, temperature, thinkingConfig, attempt: 0 });
    } catch (err) {
      lastError = err;
