...
export async function searchSimilarChunks(userId, queryText, documentIds = [], limitCount = 10) {
  ...
  try {
    ...
    let allMatches = [];
    const perDocTopK = Math.max(5, Math.ceil(limitCount / documentIds.length) + 2);

    // Query per document — Vectorize filter $in not universally supported
        ...
        try {
          ...
          if (res.ok) {
              ...
              if (data.matches && Array.isArray(data.matches)) {
                  // Only include matches with metadata (text_content must exist)
                  const validMatches = data.matches.filter(m =>
                      m.metadata && m.metadata.text_content && m.metadata.text_content.length > 10
                  );
    ...
    // Filter minimum relevance threshold (0.3 untuk menghindari hasil tidak relevan)
    const relevantMatches = allMatches.filter(m => (m.score ?? 0) >= 0.3);
    console.log(`[RAG] After threshold filter (≥0.3): ${relevantMatches.length} matches`);

    // Format kembali agar sesuai dengan struktur UI
