...
export function serializeLocalImages(html) {
  if (typeof window === "undefined" || !html) return html;
  const base64Regex = /src="data:image\/(png|jpeg|webp|gif);base64,([^"]+)"/g;
  return html.replace(base64Regex, (match, type, base64) => {
    const key = `skripzy_img_${Math.random().toString(36).substr(2, 9)}_${Date.now()}`;
    const fullBase64 = `data:image/${type};base64,${base64}`;
    try {
      localStorage.setItem(key, fullBase64);
      return `src="local-img://${key}"`;
    } catch (e) {
      console.warn("localStorage quota exceeded, keeping image inline as fallback.", e);
      return match;
    }
  });
}

export function deserializeLocalImages(html) {
  if (typeof window === "undefined" || !html) return html;
  const localImgRegex = /src="local-img:\/\/([^"]+)"/g;
  return html.replace(localImgRegex, (match, key) => {
    try {
      const base64 = localStorage.getItem(key);
      return base64 ? `src="${base64}"` : `src=""`;
    } catch (e) {
      console.warn("Failed to retrieve image from localStorage.", e);
      return `src=""`;
    }
  });
}
